...
describe('RandomPrompt', () => {
	it('renders generation controls without an active generation status', () => {
		...
		expect(screen.getByRole('button', { name: 'Random Sparks' })).toBeVisible();
		expect(
			screen.queryByText(
				/Making three sparks|Three new sparks are ready|Could not make sparks/
			)
		).not.toBeInTheDocument();
	});

	it('withholds a fast batch until the reveal while keeping existing cards visible', async () => {
		// Fake only the timeout that implements the 650ms minimum loading stage;
		...
		// The remote already resolved and wrote the store, but the 650ms minimum
