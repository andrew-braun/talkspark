...
| **Lever accents** | `--lever-who`, `--lever-topic`, `--lever-goal`, `--lever-vibe`, `--lever-depth`, `--lever-safety` | One hue per generation lever (alias onto the accent palette). A lever's chips, dot, and sentence word all share its colour. `--chip-selected-ink` is the near-black text on a filled chip; `--chip-muted-fill` is the neutral highlight for a selected "Default" chip |
